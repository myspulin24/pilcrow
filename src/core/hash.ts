 ...
 *
 * Implemented directly rather than via WebCrypto because it has to be
 * synchronous (the editor hashes on every keystroke to decide whether a buffer
 * is dirty) and because it must produce exactly the same hex digest as the
 * Rust side, which is what conflict detection compares.
 */
...
export function sha256Bytes(bytes: Uint8Array): string {
  ...
  // Length as a 64-bit big-endian integer; the high word is safe to compute in
  // floating point for any input a text editor will ever hold.
